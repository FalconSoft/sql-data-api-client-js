...
import { PrimitivesObject, PrimitiveType, ScalarObject, ScalarType, Table, TableDto } from 'datapipe-js';
...
export class SqlDataApi {
	...
	async runQuery(tableOrViewName?: string,
		fieldsOrQuery?: string | SqlReadQueryInfo,
		queryInfoSettings?: SqlReadQueryInfo): Promise<ScalarObject[]> {
	...
	async queryToTable(tableOrViewName: string, fieldsOrQuery?: string | SqlReadQueryInfo,
		queryInfoSettings?: SqlReadQueryInfo): Promise<Table<PrimitiveType>> {
		return await this._queryTable(tableOrViewName, fieldsOrQuery, queryInfoSettings);
	}
	...
	private async _query(tableOrViewName: string, fieldsOrQuery?: string | SqlReadQueryInfo, queryInfoSettings?: SqlReadQueryInfo): Promise<ScalarObject[]> {
		const table = await this._queryTable(tableOrViewName, fieldsOrQuery, queryInfoSettings);
		return fromTable(table);
	}

	private async _queryTable(tableOrViewName: string,
		fieldsOrQuery?: string | SqlReadQueryInfo,
		queryInfoSettings?: SqlReadQueryInfo): Promise<Table<PrimitiveType>> {
...
export function httpRequest<TRequest, TResponse>(method: 'GET' | 'POST' | 'PUT' | 'DELETE', url: string,
	body?: TRequest, config?: Record<string, any>): Promise<TResponse> {
